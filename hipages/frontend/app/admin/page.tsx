...
export default function AdminPanel() {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          <p style={{ fontSize: 13, color: C.ink4, margin: 0 }}>{total} item{total !== 1 ? "s" : ""} pending review</p>
          <button onClick={() => load("verification")} style={btnS(C.panel, C.ink3)}>Refresh</button>
        </div>

        {/* Profile approvals */}
        {(verification?.profiles?.length ?? 0) > 0 && (
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.ink4, textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 10px" }}>
              Tradie profile approvals ({verification!.profiles.length})
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {verification!.profiles.map(p => (
                <div key={p.id} style={{ ...panelS, padding: "14px 16px", display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 180 }}>
                    <p style={{ fontSize: 13.5, fontWeight: 700, color: C.ink, margin: "0 0 3px" }}>{p.business_name || p.full_name}</p>
                    <p style={{ fontSize: 12, color: C.ink4, margin: "0 0 4px" }}>{p.tradie_email} · {p.suburb}, {p.state}</p>
                    {p.abn && <p style={{ fontSize: 11.5, color: C.ink4, margin: 0 }}>ABN: {p.abn}</p>}
                    <p style={{ fontSize: 11, color: C.ink4, margin: "4px 0 0" }}>Submitted {timeAgo(p.created_at)}</p>
                  </div>
        ...
        {/* Certifications */}
        {(verification?.certifications?.length ?? 0) > 0 && (
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.ink4, textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 10px" }}>
              Licence certifications ({verification!.certifications.length})
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {verification!.certifications.map(c => (
                <div key={c.id} style={{ ...panelS, padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
                    <div style={{ width: 38, height: 38, borderRadius: 10, background: C.amberL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <FileText size={18} color={C.amber} />
                    </div>
                    <div style={{ flex: 1, minWidth: 180 }}>
                      <p style={{ fontSize: 13.5, fontWeight: 700, color: C.ink, margin: "0 0 3px" }}>{c.business_name}</p>
                      <p style={{ fontSize: 12, color: C.ink4, margin: "0 0 4px" }}>{c.tradie_email}</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 14px", marginTop: 4 }}>
                        {[
                          c.category_name && `Category: ${c.category_name}`,
                          c.licence_number && `Licence: ${c.licence_number}`,
                          c.issuing_state && `State: ${c.issuing_state}`,
                      ...
                      {c.edit_request_note && (
                        <div style={{ marginTop: 8, padding: "7px 10px", background: C.amberL, borderRadius: 7, border: `1px solid ${C.amber}30` }}>
                  ...
                  <p style={{ fontSize: 11, color: C.ink4, margin: "10px 0 0" }}>Submitted {timeAgo(c.created_at)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Insurance */}
        {(verification?.insurance?.length ?? 0) > 0 && (
          <div>
            <p style={{ fontSize: 11, fontWeight: 700, color: C.ink4, textTransform: "uppercase", letterSpacing: "0.12em", margin: "0 0 10px" }}>
              Insurance policies ({verification!.insurance.length})
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {verification!.insurance.map(p => (
                <div key={p.id} style={{ ...panelS, padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
                    <div style={{ width: 38, height: 38, borderRadius: 10, background: C.greenL, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <Shield size={18} color={C.green} />
                    </div>
                    <div style={{ flex: 1, minWidth: 180 }}>
                      <p style={{ fontSize: 13.5, fontWeight: 700, color: C.ink, margin: "0 0 3px" }}>{p.business_name}</p>
                      <p style={{ fontSize: 12, color: C.ink4, margin: "0 0 4px" }}>{p.tradie_email}</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 14px", marginTop: 4 }}>
                        {[
                          p.insurance_type && `Type: ${p.insurance_type?.replace(/_/g, " ")}`,
                  ...
                  <p style={{ fontSize: 11, color: C.ink4, margin: "10px 0 0" }}>Submitted {timeAgo(p.created_at)}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  const renderDisputes = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {disputes.map(d => (
        <div key={d.id} style={{ ...panelS, padding: "16px" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: C.ink, margin: "0 0 4px" }}>{d.title || "Untitled job"}</p>
              <p style={{ fontSize: 12, color: C.ink4, margin: "0 0 6px" }}>{d.suburb}, {d.state} · Last updated {timeAgo(d.updated_at)}</p>
              {d.completion_note && <p style={{ fontSize: 12.5, color: C.ink3, margin: 0, lineHeight: 1.55 }}>Note: {d.completion_note}</p>}
            </div>
